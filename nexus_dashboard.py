...
st.title("🌐 Nexus Route: Consolidation & Live Tracking")
...
def deploy_swarm(num_fleets=15): 
    fleets = []
    attempts = 0 
    
    while len(fleets) < num_fleets and attempts < 30:
        attempts += 1
        start_zone = random.choice(MUMBAI_ZONES)
        dest_zone = random.choice(MUMBAI_ZONES)
        if start_zone['name'] == dest_zone['name']: continue
            
        start = (start_zone['coords'][0] + random.uniform(-0.005, 0.005), start_zone['coords'][1] + random.uniform(-0.005, 0.005))
        dest = (dest_zone['coords'][0] + random.uniform(-0.005, 0.005), dest_zone['coords'][1] + random.uniform(-0.005, 0.005))
        
        path, dist, eta = router.calculate_traffic_route(start, dest)
        
        if path and len(path) > 5:
            pkg = random.choice(["Medical", "Hardware", "Documents"])
            prio = 1 if pkg == "Medical" else random.choice([2, 3])
            fleets.append({
                "id": f"AX-{len(fleets)+1:02d}", "pkg": pkg, "prio": prio,
                "fuel": random.randint(30, 100), "dest": dest, "zone": dest_zone['name'], # Broader fuel range for testing swaps
...
with col1:
    if st.button("🚀 Launch AI Logistics", type="primary"):
        with st.spinner("AI calculating intelligent routes..."):
            st.session_state.activity_log = []
            st.session_state.fleets = deploy_swarm(15)
            st.session_state.running = True
            log_event("SYSTEM ONLINE: 15 Fleets Deployed via TomTom Live API.")

track_target = "Show All"
if st.session_state.get('running', False):
    fleet_ids = ["Show All"] + [f['id'] for f in st.session_state.fleets]
    with col2:
        track_target = st.selectbox("🎯 Isolate & Track Fleet (View Ghost Paths):", fleet_ids)

map_view, data_view = st.columns([2.5, 1.5])

if st.session_state.get('running', False):

    # 1. ADVANCED MARL LOGIC (Using the new Policy Engine)
    for i in range(len(st.session_state.fleets)):
        for j in range(i + 1, len(st.session_state.fleets)):
            ...
            if f1['status'] == "Delivered" or f2['status'] == "Delivered": continue
            ...
            if action == "MERGE":
                ...
                f2['history'].append([[lon, lat] for lat, lon in f2['route'][f2['current_step']:]])
                ...
                f1['current_step'], f1['progress'], f2['current_step'], f2['progress'] = 0, 0.0, 0, 0.0
                f1['status'], f2['status'] = "Swapped! 🔄", "Swapped! 🔄"
                f1['reroutes'] += 1; f2['reroutes'] += 1
    ...
    for f in st.session_state.fleets:
        ...
        if f['current_step'] < len(f['route']) - 1:
            ...
            route_width = 6 if (is_tracked and track_target != "Show All") else 3
            ...
            if is_tracked and track_target != "Show All":
                for old_path in f['history']:
                    ghost_routes.append({"path": old_path, "color": [0, 255, 50, 255], "width": 5})
            
            vehicles.append({
                "unit_id": f['id'], "position": [current_lon, current_lat], 
                "fill": [255, 255, 255, opacity], "outline": [0, 0, 0, opacity] 
            })
            dest_pins.append({
    ...
    ghost_layer = pdk.Layer("PathLayer", id="ghosts", data=ghost_routes, get_path="path", get_color="color", get_width="width", width_min_pixels=4)
    route_layer = pdk.Layer("PathLayer", id="routes", data=zomato_routes, get_path="path", get_color="color", get_width="width", width_min_pixels=3, pickable=True, auto_highlight=True)
    vehicle_layer = pdk.Layer("ScatterplotLayer", id="cars", data=vehicles, get_position="position", get_fill_color="fill", get_line_color="outline", stroked=True, line_width_min_pixels=2, get_radius=50, radius_min_pixels=8, pickable=True)
    dest_layer = pdk.Layer("TextLayer", id="destinations", data=dest_pins, get_position="position", get_text="icon", get_size=30, get_color="color", get_alignment_baseline="'bottom'", pickable=True)
    ...
    if track_target != "Show All":
        ...
        view_state = pdk.ViewState(latitude=cam_lat, longitude=cam_lon, zoom=15.5, pitch=50)
    else:
        view_state = pdk.ViewState(latitude=center_lat, longitude=center_lon, zoom=11.5, pitch=40)
    ...
    with data_view:
        ...
        with log_container:
            for log_msg in st.session_state.activity_log:
                # Color coding the specific actions
                if "FUEL EFFICIENCY" in log_msg:
                    st.markdown(f"**<span style='color:#FF007F;'>{log_msg}</span>**", unsafe_allow_html=True) # Bright Pink for Fuel Swaps
                elif "CONSOLIDATION" in log_msg:
                    st.markdown(f"**<span style='color:#00BFFF;'>{log_msg}</span>**", unsafe_allow_html=True) # Bright Blue for Merges
                elif "REROUTE" in log_msg:
                    st.markdown(f"**<span style='color:#00FFAA;'>{log_msg}</span>**", unsafe_allow_html=True) # Neon Green for Reroutes
                else:
